...
async function main() {
  ...
  if (balance === 0n) throw new Error('Deployer has no balance. Get WND from the faucet first.');
  ...
  const mockUSDCAddress = await deployContract(
    wallet,
    path.join(artifactsBase, 'MockUSDC.sol/MockUSDC.json')
  );
  ...
  const registryAddress = await deployContract(
    wallet,
    path.join(artifactsBase, 'ReputationRegistry.sol/ReputationRegistry.json')
  );
  ...
  const factoryAddress = await deployContract(
    wallet,
    path.join(artifactsBase, 'SusuFactory.sol/SusuFactory.json'),
    [registryAddress, mockUSDCAddress]
  );
  ...
  const wireTx = await wallet.sendTransaction({
    to: registryAddress,
    data: setFactoryData,
    gasLimit: GAS_LIMIT,
    gasPrice: GAS_PRICE,
  ...
  const factoryArtifact = JSON.parse(
    fs.readFileSync(path.join(artifactsBase, 'SusuFactory.sol/SusuFactory.json'), 'utf8')
  );
  ...
  const deploymentData = {
    ...
    contracts: {
      MockUSDC: {
        ...
        abi: JSON.parse(fs.readFileSync(path.join(artifactsBase, 'MockUSDC.sol/MockUSDC.json'), 'utf8')).abi,
      },
      ReputationRegistry: {
      ...
      SusuPool: {
        ...
        abi: JSON.parse(fs.readFileSync(path.join(artifactsBase, 'SusuPool.sol/SusuPool.json'), 'utf8')).abi,
      },
    },
  };
